...
function bindSlideToggle(trigger, boxbody, content, openClass) {
    ...
    button.element.addEventListener('click', () => {
        if (button.active === false) { //Проверяем меню на неактивность
            button.active = true; //Если она не активна - мы делаем её активной
...
//         card.classList.add('videos__item', 'videos__item-active'); //Создаёт класс 
//         card.setAttribute('data-url', data[3][i]); //Устанавливает атрибуты 
//         card.innerHTML = `
...
function start() {
    gapi.client.init({
        'apiKey': 'AIzaSyCDBF2fToc2qclnsICPkAfHKHsP8zoOVwc',
        ...
        response.result.items.forEach(item => {
            let card = document.createElement('a');
                    ...
                    card.classList.add('videos__item', 'videos__item-active'); //Создаёт класс 
                    card.setAttribute('data-url', item.contentDetails.videoId); //Устанавливает атрибуты 
                    card.innerHTML = `
    ...
    gapi.client.init({
        ...
        response.result.items.forEach(item => {
            let card = document.createElement('a');
                    ...
                    card.classList.add('videos__item', 'videos__item-active'); //Создаёт класс 
                    card.setAttribute('data-url', item.id.videoId); //Устанавливает атрибуты 
                    card.innerHTML = `
                    <img src="${item.snippet.thumbnails.high.url}" alt="thumb">
                    <div class="videos__item-descr">
                        ${item.snippet.title}
...
modal.addEventListener('click', (e) => {
    if (!e.target.classList.contains('modal__body')) {
        ...
        // if(e.which === 30) Если нажатие происходит на кнопку с номером 30, модальное окно закрывается
        // closeModal();
    } 
});
...
//AIzaSyCDBF2fToc2qclnsICPkAfHKHsP8zoOVwc
